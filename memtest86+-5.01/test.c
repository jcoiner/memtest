...
//  relocated) global elf tables. This avoids the crashes.
//
//  Compiling with -O0 can also avoid the crashes, they only seem
//  to happen compiling test.c with optimization.
...
(ulong* start, ulong* end,
 int me, const void* ctx, segment_fn func) {
	...
	do {
		...
		// ensure no overflow
		ASSERT((seg_end_dw + SPINSZ_DW) > seg_end_dw);
		seg_end_dw += SPINSZ_DW;
...
/* Calls segment_fn() for each segment in v->map.
 *
...
STATIC void addr_tst1_seg(ulong* buf,
	...
	//    Segment of cpu-visible memory map to a banks of physical memory
	//    hardware. By sweeping 'off' across the address space, we hope to
	//    catch stuck address lines within individual banks.
	...
	for (pat=0x5555aaaa, k=0; k<2; k++) {
		...
		for (ulong off_dw = 0; off_dw < len_dw; off_dw += (1 << 18)) {
			...
			for (ulong alias_dw = 1; off_dw + alias_dw < len_dw;
			     alias_dw = alias_dw << 1) {
				ASSERT(alias_dw);  // it should never get to zero
				buf[off_dw + alias_dw] = pat;
				ulong bad;
			...
			for (ulong alias_dw = 1; off_dw > alias_dw;
			     alias_dw = alias_dw << 1) {
				ASSERT(alias_dw);  // it should never get to zero
				buf[off_dw - alias_dw] = pat;
				ulong bad;
